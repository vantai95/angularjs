...
function LvTestController($ksHttp, $scope, $state, $uibModal, $rootScope, $stateParams) {
			...
			$ksHttp.post('LevelTestResultSave', params).then(function(rs) {
				...
				if(status == 'succ') {
					if( "I" == params.saveType){ //중복저장 방지
						$ksHttp.post('GetLevelTestResult', {ltCd : $scope.ltCd}).then(function(rs) {
							rs = JSON.parse(rs);
							$scope.lvtest_list = rs;
							$scope.initPopup();
						}, function(error) {
