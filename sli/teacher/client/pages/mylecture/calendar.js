...
app.controller('FullcalendarCtrl', ['$scope', '$rootScope', function($scope, $rootScope) {
    ...
    if($scope.popup5_attendStudent_list.length == 0){
    	$scope.latest_date = moment().format('YYYY-MM-DD');
    }
    
    $scope.events = [];
    angular.forEach($scope.popup5_attendStudent_list, function(ele){
    ...
    $scope.getStudentInfo = function(selected_date){
    	var bExists = false;
    	for(var i = 0; i < $scope.popup5_attendStudent_list.length; i++) {
    		if($scope.popup5_attendStudent_list[i].attDate == selected_date) {
    			bExists = true;
    		}
    	}
    	...
    	var attend_students = $.grep($scope.popup5_attendStudent_list, function(x, i){
    	...
    	//console.log($scope.schedule_item);
    	setTimeout(function(){
...
//      $scope.lastClickTime = time;
	...
	$scope.setStatus = function(value) {
		$scope.schedule_item.attState = value;
	}
